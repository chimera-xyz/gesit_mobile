...
class LeavePdfPreviewScreen extends StatefulWidget {
  const LeavePdfPreviewScreen({
    super.key,
  ...
  @override
  State<LeavePdfPreviewScreen> createState() => _LeavePdfPreviewScreenState();
}

class _LeavePdfPreviewScreenState extends State<LeavePdfPreviewScreen> {
  Uint8List? _pdfBytes;
  ...
  Future<void> _loadPdf() async {
    ...
    try {
      final bytes = await widget.controller.fetchLeavePdfPreview(
        widget.request,
      );
      if (!mounted) {
        return;
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('PDF Cuti', style: textTheme.headlineSmall),
                            const SizedBox(height: 4),
                            Text(
                              'CUTI-${widget.request.id}.pdf',
                              maxLines: 1,
                      ...
                      child: ClipRRect(
                        ...
                        child: _loading
                            ...
                            : PdfViewer.data(
                                pdfBytes,
                                sourceName:
                                    'cuti-${widget.request.id}-${pdfBytes.length}.pdf',
                              ),
                      ),
                    ),
                  ),
